...
import com.example.shop.domain.model.ProductProperty
import com.example.shop.ui.theme.Gray
import com.example.shop.ui.theme.LightBlue
import com.example.shop.data.utils.setProductImage
import com.example.shop.presentation.admin.viewmodel.AdminViewModel
import com.example.shop.presentation.seller.viewmodel.CreateProductViewModel
...
import com.example.shop.presentation.seller.viewmodel.AnalyzeViewModel
import com.example.shop.ui.theme.TransientGray
...
fun CreateProduct(navController: NavController, viewModel: MainViewModel) {
    ...
    val vm = AnalyzeViewModel(context)
    val meta = vm.meta.collectAsState()
    val loading by vm.loading.collectAsState()
    val error by vm.error.collectAsState()
    val adminViewModel :AdminViewModel = hiltViewModel()
    val categories = adminViewModel.categories.observeAsState()
    val createProductViewModel :CreateProductViewModel= hiltViewModel()
    ...
    var imUri = remember { mutableStateOf<Uri?>(null) }
    ...
    val launcher = setProductImage(context, imageUrl, newProduct, imUri)
    ...
    if (meta.value != null) {
        Log.d("TESTPROP", meta.value.toString())

        meta.value?.характеристики?.forEach { (key, value) ->
            if (key.isNotBlank() && value.isNotBlank()) {
                val newProperty = ProductProperty(
                    prop = key.replaceFirstChar { it.titlecase() },
                    value = value.replaceFirstChar { it.titlecase() }
                )
                Log.d("TESTPROP", newProperty.toString())

                if (!properties.value.any { it.prop == newProperty.prop }) {
                    properties.value.add(newProperty)
                }
            }
        }
        Log.d("TESTPROP", properties.value.toString())
    }
    Scaffold(
        topBar = {
        ...
        if (currentUser.value == null) {
            ...
            Box(
                ...
                Column(
                    ...
                    Text(text = "Генерация характеристик...", color = LightBlue, fontSize = 20.sp)
                }
...
//                                        .align(Alignment.Start)
                                ...
                                TextButton(
                                    onClick = { vm.analyze(imUri.value!!) },
                                    ...
                                    Row(
                                        ...
                                        Icon(
                                            painter = painterResource(R.drawable.generate_ai_icon),
                                            contentDescription = null,
                                            modifier = Modifier.size(50.dp),
                                            tint = LightBlue
                                        )
                                        Text(text = "AI help", color = LightBlue, fontSize = 20.sp)
