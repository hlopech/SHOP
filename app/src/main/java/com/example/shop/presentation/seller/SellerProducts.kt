...
import com.example.shop.presentation.common.viewmodel.ProductViewModel
...
@Composable
fun SellerProducts(navController: NavController, viewModel: MainViewModel) {
    val currentUser = viewModel.currentUser.observeAsState()
    val productViewModel :ProductViewModel= hiltViewModel()
    val products = productViewModel.products.observeAsState()

    productViewModel.getProductsBySellerId(currentUser.value?.uid)
    ...
    LaunchedEffect(products.value) {
        if (products.value != null) {
            ...
            val productsWithSales = products.value?.filter { it?.sales != 0 } ?: emptyList()
    ...
    Scaffold(
        topBar = {
            CenterAlignedTopAppBar(
                title = { Text("Мои товары", color = MaterialTheme.colorScheme.secondary) },
                navigationIcon = {
                ...
                actions = {
                    IconButton(onClick = { navController.navigate("sellerProducts") }) {
                        Icon(
                    ...
                    DropdownMenu(
                        ...
                        DropdownMenuItem(
                            ...
                            onClick = { sortsShowBottomSheet = true })
                        ...
                        DropdownMenuItem(
                            ...
                            onClick = { filtersShowBottomSheet = true })
    ...
    {
        if (currentUser.value == null || products.value == null) {
